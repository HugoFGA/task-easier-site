...
(function () {
  ...
  // onProgress (opcional) é avisado assim que cada página/lista termina de
  // carregar, com o array acumulado até aquele ponto — permite que a tela
  // mobile vá exibindo as tarefas aos poucos em vez de esperar tudo.
  async function getTasks(incluirConcluidas, onProgress) {
    ...
    for (var i = 0; i < listas.length; i++) {
      ...
      do {
        ...
        // Ainda falta buscar mais (outra página ou outra lista) — avisa com
        // o que já temos. O retorno final da função cobre o último trecho.
        if ((pageToken || i < listas.length - 1) && typeof onProgress === 'function') {
          onProgress(result.slice());
        }
      } while (pageToken);
    }
    return result;
  ...
  async function listCalendarEvents(timeMinIso, timeMaxIso, onPage) {
    var items = [];
    var pageToken = null;
    do {
      var params = new URLSearchParams({
      ...
      pageToken = (resp && resp.nextPageToken) || null;
      if (pageToken && typeof onPage === 'function') onPage(items.slice());
    } while (pageToken);
    return items;
  }
  ...
  async function getEvents(incluirConcluidas, onProgress) {
    var now = new Date();
    ...
    var items = await listCalendarEvents(start.toISOString(), end.toISOString(), function (partialRaw) {
      if (typeof onProgress === 'function') onProgress(process(partialRaw));
    });
