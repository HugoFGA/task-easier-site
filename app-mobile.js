...
//
// Tarefas e eventos vêm de duas chamadas separadas (getTasks/getEvents).
// Em vez de esperar as duas terminarem para mostrar qualquer coisa (como
// era antes, com Promise.all), cada uma popula a tela assim que responde
// — a lista vai aparecendo aos poucos, sem a sensação de travar enquanto
// a mais lenta das duas ainda está carregando.
...
function loadTasksView() {
  ...
  gsCall('getTasks', [showAllMode])
    .then(function(tasks) { tasksResult = tasks || []; render(); })
    .catch(function(e) {
  ...
  gsCall('getEvents', [showAllMode])
    .then(function(events) { eventsResult = events || []; render(); })
    .catch(function(e) {
      if (requestId === loadTasksRequestId) showToast(T('error_loading_prefix') + e.message, 'err');
