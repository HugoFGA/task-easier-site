// Arquivo: app-mobile.js | Versão: 14 (Task Easier — página mobile/web, fora da extensão)
// ═══════════════════════════════════════════════════════════
...
// ── BLOQUEIO DE ROLAGEM COM O TECLADO ABERTO ─────────────────────────
// No celular, ao tocar em um campo de texto o teclado abre e o navegador
// tenta rolar a página para mostrar o campo — isso deixa a janela do app
// "rolável" para cima/baixo enquanto se digita, o que não é desejado aqui
// (o formulário e a busca já ficam fixos na tela). A solução é travar a
// posição do <body> (position:fixed) assim que um campo ganha foco, e
// devolver a rolagem normal quando o foco sai do campo.
(function preventScrollWhileTyping() {
  var savedScrollY = 0;
  var locked = false;
  ...
  document.addEventListener('focusin', function(e) {
    if (isTextField(e.target)) lockScroll();
  });
  document.addEventListener('focusout', function(e) {
    // Pequeno atraso: se o usuário está trocando de um campo para outro
    // (ex.: Tab/Próximo do teclado), o focusin do novo campo já dispara
    // antes deste timeout — assim evita destravar e travar de novo à toa.
    setTimeout(function() {
      if (!isTextField(document.activeElement)) unlockScroll();
    }, 50);
  });
})();
...
const I18N = {
  pt: {
    ...
    toast_export_ok: 'Backup exportado!',
    toast_importing: 'Importando…',
    toast_import_invalid: 'Arquivo inválido',
    toast_import_done: 'Importação concluída: {ok} criados, {fail} com erro'
  },
  en: {
    ...
    toast_export_ok: 'Backup exported!',
    toast_importing: 'Importing…',
    toast_import_invalid: 'Invalid file',
    toast_import_done: 'Import finished: {ok} created, {fail} failed'
  }
};
...
(function setupDataMenu() {
  var btn = document.getElementById('data-menu-btn');
  ...
  fileInput.addEventListener('change', function(e) {
    ...
    reader.onload = function() {
      ...
      try { data = JSON.parse(reader.result); } catch (err) {
        showToast(T('toast_import_invalid'), 'err');
        return;
      }
      var items = Array.isArray(data) ? data : (data && Array.isArray(data.items) ? data.items : null);
      if (!items) { showToast(T('toast_import_invalid'), 'err'); return; }
      showToast(T('toast_importing'), 'ok');
      importItemsFromJson(items).then(function(res) {
        showToast(T('toast_import_done').replace('{ok}', res.okCount).replace('{fail}', res.failCount), res.failCount ? 'err' : 'ok');
        loadTasksView();
      });
    };
...
// Só tarefas e eventos PENDENTES entram no export/import — os já
// concluídos não fazem parte do backup.
async function exportAllData() {
  try {
    var tasks = await gsCall('getTasks', [false]);
    var events = await gsCall('getEvents', [false]);
    var payload = {
      app: 'Task Easier',
      exportedAt: new Date().toISOString(),
      version: 1,
      items: sortItems((tasks || []).concat(events || [])).map(function(t) {
        return {
          type: t.type,
          title: t.title,
          due: t.due || '',
          dueTime: t.dueTime || '',
          notes: t.notes || '',
          color: t.type === 'event' ? (t.color || DEFAULT_EVENT_COLOR) : undefined
        };
      })
    };
    ...
    a.href = url;
    a.download = 'task-easier-' + (todayISO || 'backup') + '.json';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    showToast(T('toast_export_ok'), 'ok');
  } catch (e) {
    showToast(T('error_prefix') + e.message, 'err');
  }
}

// Recria, como pendentes, os itens de um JSON exportado (ou compatível)
// pela própria função acima. Segue em frente mesmo se um item falhar,
// contando acertos/erros para mostrar um resumo no final.
async function importItemsFromJson(items) {
  var listId = resolveMainListId();
  var okCount = 0, failCount = 0;
  // Encadeia eventos do mesmo dia 15 min após o anterior (mesma lógica usada
  // ao criar vários eventos de uma vez), evitando que fiquem todos no mesmo
  // horário quando o arquivo importado tem horários iguais ou ausentes.
  var lastMinutesByDate = {};
  for (var i = 0; i < items.length; i++) {
    var it = items[i] || {};
    try {
      if (it.type === 'event') {
        var dp = (it.due || '').split('-').map(Number);
        var tp = (it.dueTime || '09:00').split(':').map(Number);
        if (!dp[0] || !dp[1] || !dp[2]) throw new Error('data inválida');
        var dateKey = it.due;
        var minutes = (tp[0] || 9) * 60 + (tp[1] || 0);
        if (lastMinutesByDate[dateKey] !== undefined && minutes <= lastMinutesByDate[dateKey]) {
          minutes = lastMinutesByDate[dateKey] + 15;
        }
        lastMinutesByDate[dateKey] = minutes;
        var hh = Math.floor(minutes / 60) % 24;
        var mm = minutes % 60;
        await gsCall('createEventQuick', [
          it.title || '(sem título)', dp[0], dp[1], dp[2],
          hh, mm, it.notes || '', it.color || DEFAULT_EVENT_COLOR
        ]);
      } else {
        if (!listId) throw new Error('nenhuma lista de tarefas encontrada');
        await gsCall('createTask', [listId, it.title || '(sem título)', it.due || null, it.notes || null]);
      }
      okCount++;
    } catch (e) {
      failCount++;
...
async function startApp() {
  ...
  document.getElementById('data-menu-btn').classList.remove('hidden');
  updatePeekButtonTitle();
  loadTasksView();
  titleInput.focus();
}
