// Arquivo: app-mobile.js | Versão: 3 (Task Easier — página mobile/web, fora da extensão)
// ═══════════════════════════════════════════════════════════
...
const I18N = {
  pt: {
    ...
    toast_deleted_undo: 'Excluído. Toque para desfazer.',
    toast_deleted_undo_n: '{n} itens excluídos. Toque para desfazer.',
  ...
  en: {
    ...
    delete_title: 'Delete',
    toast_deleted_undo: 'Deleted. Tap to undo.',
    toast_deleted_undo_n: '{n} items deleted. Tap to undo.',
    btn_add_title: 'Add'
  }
};
...
// ── BANNER NO TOPO (confirmação de criação / desfazer exclusão) ──────
...
function showToast(msg, type, duration, onClick) {
  ...
  clearTimeout(toastHideTimer);
  toastHideTimer = setTimeout(hideToast, duration || 2200);
}

// ── EXCLUSÃO COM "DESFAZER" (5 segundos) ─────────────────────────────
// A linha some da tela na hora (otimista), mas a exclusão de verdade no
// Google só acontece depois de 5s — se a pessoa tocar no banner antes
// disso, a linha volta e nada é apagado.
var pendingUndos = [];
...
function updateUndoToast() {
  ...
  if (!n) { hideToast(); return; }
  var msg = n === 1 ? T('toast_deleted_undo') : tf('toast_deleted_undo_n', { n: n });
  showToast(msg, 'undo', 5000, function() {
...
function scheduleRowDelete(row, listEl, doDeleteFn) {
  ...
  entry.timer = setTimeout(function() {
    var idx = pendingUndos.indexOf(entry);
    if (idx > -1) pendingUndos.splice(idx, 1);
    doDeleteFn().catch(function(e) { showToast(T('error_prefix') + e.message, 'err'); });
    if (!pendingUndos.length) hideToast();
  }, 5000);
...
function renderTasksList(items) {
  ...
  items.forEach(function(t) {
    ...
    chk.addEventListener('change', function() {
      ...
      call
        .then(function() {
          // Só tarefas/eventos pendentes ficam na lista — ao concluir, some.
          if (willBeDone) {
            row.remove();
            checkEmptyState();
          } else {
    ...
    delBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      scheduleRowDelete(row, listEl, function() {
...
// ordenados por data/hora (os sem data ficam no fim) — pensada para ficar
// sempre visível, rolando de forma independente do formulário fixo no topo.
...
// ── ALTURA DA LISTA ROLÁVEL ──────────────────────────────────────────
// Calcula na mão o espaço disponível abaixo do formulário fixo e fixa essa
// altura em pixels na lista — mais confiável do que depender só de
// flex/overflow em navegadores/Android mais antigos.
function fitTasksView() {
  var appView = document.getElementById('app-view');
  var tv = document.getElementById('tasks-view');
  if (!appView || !tv || appView.classList.contains('hidden')) return;
  var rect = tv.getBoundingClientRect();
  var vh = (window.visualViewport && window.visualViewport.height) || window.innerHeight;
  var available = vh - rect.top - 12;
  tv.style.flex = 'none';
  tv.style.height = Math.max(120, Math.floor(available)) + 'px';
}
window.addEventListener('resize', fitTasksView);
window.addEventListener('orientationchange', function() { setTimeout(fitTasksView, 200); });
if (window.visualViewport) window.visualViewport.addEventListener('resize', fitTasksView);
